...
# Find the curb location with highest damage rate
...
feature_sets <- list(
  ...
  "+ dbh_area"        = update(base_formula, ~ . + dbh_area),
  "+ dbh_log"         = update(base_formula, ~ . + dbh_log),
  "+ dbh_sqrt"        = update(base_formula, ~ . + dbh_sqrt),
  "+ dbh_x_curb"      = update(base_formula, ~ . + dbh_x_curb),
  "+ all size"        = update(base_formula, ~ . + dbh_area + dbh_log + dbh_sqrt + dbh_x_curb)
)
...
# ------------------------------
cat("\n========================================\n")
cat("Size Transformations – F1 threshold (0.46)\n")
print(results[, c("Model", "F1_0.46", "F2_0.46", "dF1_46", "dF2_46")], row.names = FALSE)

cat("\nSize Transformations – F2 threshold (0.23)\n")
