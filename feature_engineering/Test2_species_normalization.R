...
species_list <- unique(tree_data$spc_common)
tree_data$dbh_zscore <- NA

for (sp in species_list) {
  idx <- which(tree_data$spc_common == sp)
  if (length(idx) > 1) {
    mu <- mean(tree_data$tree_dbh[idx])
    sigma <- sd(tree_data$tree_dbh[idx])
    tree_data$dbh_zscore[idx] <- (tree_data$tree_dbh[idx] - mu) / sigma
  } else {
    tree_data$dbh_zscore[idx] <- 0
  }
}

# Safety check: any remaining NAs
tree_data$dbh_zscore[is.na(tree_data$dbh_zscore)] <- 0
...
feature_sets <- list(
  ...
  "+ dbh_zscore" = update(base_formula, ~ . + dbh_zscore)
)
...
# Output
cat("\n========================================\n")
cat("Species Normalization – F1 threshold (0.46)\n")
print(results[, c("Model", "F1_0.46", "F2_0.46", "dF1_46", "dF2_46")], row.names = FALSE)

cat("\nSpecies Normalization – F2 threshold (0.23)\n")
